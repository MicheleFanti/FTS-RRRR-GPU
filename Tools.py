...
def initialize_alternative_rho(sequence, rho_class, rho0_per_class,
    ...
    for res_key in rho_class:
        base = rho0_per_class[res_key]
        # Initialize fully 3D
        rho = np.ones((Nx, Ny, Nang), dtype=np.float64) * base

        # Central droplet
        r2 = (X - center[0])**2 + (Y - center[1])**2
        gaussian = np.exp(-r2 / (2 * sigma**2))
        rho += base * droplet_boost * gaussian[..., None]  # broadcast along Nang

        # Optional noise droplets
        for _ in range(n_noise_droplets):
            ...
            rho += base * droplet_boost * noise_boost_frac * gaussian_noise[..., None]

        # Normalize weighted integral
        integral = np.sum(spat_weights[..., None] * ang_weights[None, None, :] * rho)
        if integral > 0:
            rho *= base * np.sum(spat_weights) * np.sum(ang_weights) / integral
        else:
            rho *= 0.0
...
def build_initials(sequence, l_chain, rhop0, gridshape, spat_weights, ang_weights):
    ...
    rho_class = initialize_alternative_rho(sequence, rho_class, rho0_per_class, gridshape, spat_weights, ang_weights)
    ...
    return residue_classes_per_s, residue_classes, rho0_per_class, rho_backbone, rho_sidechains
