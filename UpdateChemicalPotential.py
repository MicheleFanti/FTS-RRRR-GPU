...
class SCFTUpdater:
    ...
    def linear_descent(self, xi_prior, w_prior_bb, w_prior_solv, w_prior_sc, rho_bb_new, rho_solv_new, rho_sc_new, gamma, ang_weights, spat_weights,  K_hydro, K_hb, A_hb, K_es, h_as, c_field, box_length, gridshape):
        ...
        for key in w_prior_bb:
            w_new_bb[key] = w_prior_bb[key] + gamma * wP_trial[key]
        w_new_solv = {}
        for key in w_prior_solv:
            w_new_solv[key] = w_prior_solv[key] + gamma * wS_trial[key]
        w_new_sc = {}
        for key in w_prior_sc:
            w_new_sc[key] = w_prior_sc[key] + gamma*wSc_trial[key]
        ...
        solv_deviation = sum(np.sum((wS_trial[key])**2  * spat_weights) for key in wS_trial)
        print(f'Deviations being: BB:= {bb_deviation}, SOLV:=  {solv_deviation}, SC:=  {sc_deviation}')
        return w_new_bb, w_new_sc, w_new_solv, xi, [bb_deviation, sc_deviation, solv_deviation]
        
    def zero_update(self, rho_bb_new, rho_solv_new, rho_sc_new, gamma, ang_weights, spat_weights,  K_hydro, K_hb, A_hb, K_es, h_as, c_field, box_length, gridshape):
        rhoPnew = np.zeros(gridshape[:2])
        dx = box_length[0]/gridshape[0]
        rhoS_total = sum(rho_solv_new[s] for s in ["neutral","plus","minus"])
        ihatc = np.fft.ifftn(np.fft.fftn(c_field)*K_es).real * (dx**2)
        ...
        for species in rho_bb_new:
            ...
            comp_h_as = h_as_total
            comp_es = self.bjerrum_length* self.es_charges[species]*ihatc
            wP_trial[species] = gamma*np.broadcast_to(comp_vchi_ps[..., None] + comp_h_as[..., None] + comp_es[..., None] +xi[..., None], gridshape).copy()
        ...
        return wP_trial, wSc_trial, wS_trial, xi